$(document).ready(function () {
    ...
    $("#updatePrecenter").on('click', '.update', function () {
        ...
        if (confirm('are you sure?')) {
            ...
            $.ajax({
                type: 'GET',
                url: '/staff/seminar/updatePrecenter',
                data: {
                    idSeminar: idSeminar,
                    idPrecenter
                },
    ...
    $(".delPre").submit(function (e) {
        ...
        if (confirm('are you sure?')) {
            ...
            $.ajax({
                type: "POST",
                url: "/staff/seminar/delPerforment",
                data: form.serialize(),
            ...
            success: function (performent) {
                ...
                if (performent.length > 0) {
                    for (var i = 0; i < performent.length; i++) {
                        ...
                        result += '<img src = "/img/performent/' + performent[i].img + '" class="img-fluid" style = "width:100%" >';
    ...
    $("#ModalPer").on("click", ".addPerSemenar", function () {
        if (confirm('are you sure?')) {
            ...
            $.ajax({
                type: "POST",
                url: "/staff/seminar/addperforment",
                data: {
                    idSeminar: idSeminar,
